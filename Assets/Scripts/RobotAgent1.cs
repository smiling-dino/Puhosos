...
public class RobotAgent1 : Agent
...

{
    ...
    private void CalculateRewards(float gasSignal, float steerSignal)
    {
        ...
        if (virtualSensors != null && virtualSensors.isBallInGripper && targetBall != null && !isHoldingBall)
        {
            AddReward(0.01f); // Поощряем просто за то, что мяч перед носом
        } 
