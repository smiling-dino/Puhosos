    ...
    private void ControlClaw(int action)
    {
        ...
        if (action == 1) // Закрыть клешню
        {
            if (virtualSensors.isBallInGripper && targetBall != null)
            {
                ...
                if (!hasGrabbedBall)
                {
                    AddReward(10.0f); // Максимальная награда как за доставку
                    hasGrabbedBall = true; 
                    SendStatsToTensorBoard(true); // Записываем как глобальный успех
                    Debug.Log("<color=cyan>[ПОБЕДА]</color> Мяч захвачен! Изолированная задача выполнена.");
                    
                    EndEpisode(); // НОВОЕ: Сразу завершаем эпизод победой!
                }
            }
